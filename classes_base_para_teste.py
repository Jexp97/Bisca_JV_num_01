...
class MesaDeBisca():
    ...
    def adicionar_jogador(self, nome, indice):
        ...
        if (len(self._jogadores) < self._numero_de_jogadores):
            ...
            '''
            # Comentar a seguinte seção quando for jogar. Ela é usada apenas para treino
            # da rede neural.

            if indice % 2 == 0:
                # indice par adiciona Smart Player (Equipe A)
                self._jogadores.append(SmartPlayer(nome))
            else:
                # indice ínpar adiciona Dummy Player (Equipe B)
                self._jogadores.append(DummyPlayer(nome))
